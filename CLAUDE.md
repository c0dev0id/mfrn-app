...
Android companion app for the **MFRN Forum** (motorradfreunde-rheinneckar.de). Not a full forum replacement — a focused mobile client for key features.

- **Calendar** — upcoming events with date, name, author, description, participants; push notifications
- **Private Messages** — sender/receiver threading, messenger-style UI, attachment support
- **Filebase** — file listing with author/description, GPX file preview on map, download support

## Stack

- Java 17, AGP 8.5.2, Gradle 8.7
- minSdk 34 (Android 14), targetSdk 35
- OkHttp 4.12.0 for networking (cookie jar for session persistence)
- Material3 View-based UI (no Compose)

## Build

All builds run in CI/CD (GitHub Actions). Do not attempt local Android builds.
...
CI workflow: lint → build → sign → nightly pre-release on every push to `main`.
